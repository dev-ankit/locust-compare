...
def temp_yaml_files():
    ...
    # Simple file 1
    file1_data = {"database": "postgres", "port": 5432, "debug": True}

    # Simple file 2
    file2_data = {"database": "postgres", "port": 3306, "logging": "verbose"}

    # Nested file 1
    nested1_data = {"database": {"host": "localhost", "port": 5432}, "app": {"name": "myapp"}}

    # Nested file 2
    nested2_data = {"database": {"host": "localhost", "port": 3306}, "app": {"name": "myapp"}}
...
class TestFlattenDict:
    ...
    def test_flatten_depth_1(self):
        ...
        data = {"database": {"host": "localhost", "port": 5432}, "app": {"name": "myapp"}}
    ...
    def test_flatten_depth_2(self):
        ...
        expected = {"database.host": "localhost", "database.port": 5432, "app.name": "myapp"}
...
class TestUnflattenDict:
    ...
    def test_unflatten_simple(self):
        ...
        data = {"database.host": "localhost", "database.port": 5432, "app.name": "myapp"}
        result = unflatten_dict(data)
        expected = {"database": {"host": "localhost", "port": 5432}, "app": {"name": "myapp"}}
        assert result == expected
...
class TestUnionCommand:
    ...
    def test_union_kv_mode(self, runner, temp_yaml_files):
        ...
        result = runner.invoke(main, ["union", temp_yaml_files["file1"], temp_yaml_files["file2"]])
    ...
    def test_union_keys_mode(self, runner, temp_yaml_files):
        ...
        result = runner.invoke(
            main, ["union", temp_yaml_files["file1"], temp_yaml_files["file2"], "--compare", "keys"]
        )
        assert result.exit_code == 0
        output_data = yaml.safe_load(result.output)
        assert output_data["database"] == "postgres"
...
class TestIntersectCommand:
    ...
    def test_intersect_kv_mode(self, runner, temp_yaml_files):
        ...
        result = runner.invoke(
            main, ["intersect", temp_yaml_files["file1"], temp_yaml_files["file2"]]
        )
        assert result.exit_code == 0
        output_data = yaml.safe_load(result.output)
        assert output_data == {"database": "postgres"}
    ...
    def test_intersect_keys_mode(self, runner, temp_yaml_files):
        ...
        result = runner.invoke(
            main,
            ["intersect", temp_yaml_files["file1"], temp_yaml_files["file2"], "--compare", "keys"],
        )
        assert result.exit_code == 0
        output_data = yaml.safe_load(result.output)
        assert output_data["database"] == "postgres"
    ...
    def test_intersect_depth_2(self, runner, temp_yaml_files):
        ...
        result = runner.invoke(
            main,
            ["intersect", temp_yaml_files["nested1"], temp_yaml_files["nested2"], "--depth", "2"],
        )
        assert result.exit_code == 0
        output_data = yaml.safe_load(result.output)
        assert output_data == {"database": {"host": "localhost"}, "app": {"name": "myapp"}}

    def test_intersect_depth_0(self, runner, temp_yaml_files):
        """Test intersect with depth 0 (unlimited)."""
        result = runner.invoke(
            main,
            ["intersect", temp_yaml_files["nested1"], temp_yaml_files["nested2"], "--depth", "0"],
        )
        assert result.exit_code == 0
        output_data = yaml.safe_load(result.output)
...
class TestDiffCommand:
    ...
    def test_diff_kv_mode(self, runner, temp_yaml_files):
        ...
        result = runner.invoke(main, ["diff", temp_yaml_files["file1"], temp_yaml_files["file2"]])
        assert result.exit_code == 0
        output_data = yaml.safe_load(result.output)
    ...
    def test_diff_keys_mode(self, runner, temp_yaml_files):
        ...
        result = runner.invoke(
            main, ["diff", temp_yaml_files["file1"], temp_yaml_files["file2"], "--compare", "keys"]
        )
        assert result.exit_code == 0
        output_data = yaml.safe_load(result.output)
...
class TestRdiffCommand:
    ...
    def test_rdiff_kv_mode(self, runner, temp_yaml_files):
        ...
        result = runner.invoke(main, ["rdiff", temp_yaml_files["file1"], temp_yaml_files["file2"]])
        assert result.exit_code == 0
        output_data = yaml.safe_load(result.output)
    ...
    def test_rdiff_keys_mode(self, runner, temp_yaml_files):
        ...
        result = runner.invoke(
            main, ["rdiff", temp_yaml_files["file1"], temp_yaml_files["file2"], "--compare", "keys"]
        )
        assert result.exit_code == 0
        output_data = yaml.safe_load(result.output)
        assert output_data == {"logging": "verbose"}
...
class TestSymdiffCommand:
    ...
    def test_symdiff_kv_mode(self, runner, temp_yaml_files):
        ...
        result = runner.invoke(
            main, ["symdiff", temp_yaml_files["file1"], temp_yaml_files["file2"]]
        )
        assert result.exit_code == 0
        output_data = yaml.safe_load(result.output)
    ...
    def test_symdiff_keys_mode(self, runner, temp_yaml_files):
        ...
        result = runner.invoke(
            main,
            ["symdiff", temp_yaml_files["file1"], temp_yaml_files["file2"], "--compare", "keys"],
        )
        assert result.exit_code == 0
        output_data = yaml.safe_load(result.output)
...
class TestErrorHandling:
    ...
    def test_file_not_found(self, runner, temp_yaml_files):
        ...
        result = runner.invoke(
            main, ["intersect", "/nonexistent/file.yml", temp_yaml_files["file1"]]
        )
    ...
    def test_invalid_yaml(self, runner, temp_yaml_files):
        ...
        result = runner.invoke(
            main, ["intersect", temp_yaml_files["invalid"], temp_yaml_files["file1"]]
        )
    ...
    def test_non_dict_root(self, runner, temp_yaml_files):
        ...
        result = runner.invoke(
            main, ["intersect", temp_yaml_files["list"], temp_yaml_files["file1"]]
        )
        assert result.exit_code == 1
...
class TestEdgeCases:
    ...
    def test_empty_file(self, runner, temp_yaml_files):
        ...
        result = runner.invoke(main, ["union", temp_yaml_files["empty"], temp_yaml_files["file1"]])
        assert result.exit_code == 0
        output_data = yaml.safe_load(result.output)
    ...
    def test_identical_files_intersect(self, runner, temp_yaml_files):
        ...
        result = runner.invoke(
            main, ["intersect", temp_yaml_files["file1"], temp_yaml_files["identical"]]
        )
        assert result.exit_code == 0
        output_data = yaml.safe_load(result.output)
    ...
    def test_identical_files_diff(self, runner, temp_yaml_files):
        ...
        result = runner.invoke(
            main, ["diff", temp_yaml_files["file1"], temp_yaml_files["identical"]]
        )
        assert result.exit_code == 0
        output_data = yaml.safe_load(result.output)
    ...
    def test_no_matches_intersect(self, runner, temp_yaml_files):
        ...
        try:
            result = runner.invoke(main, ["intersect", temp_yaml_files["file1"], different_file])
            assert result.exit_code == 0
            output_data = yaml.safe_load(result.output)
            # Should return empty
            assert output_data == {} or output_data is None
